...
//Quick add Rak
$(document).on('submit', 'form#quick_add_rak_form', function(e){
	e.preventDefault();
	var data = $(this).serialize();
	...
	$.ajax({
		...
		success: function(result){
			if(result.success == true){
				var newOption = new Option(result.data.tempat_simpan, result.data.id, true, true);
			    // Append it to the select
			    $('#rak_id').append(newOption).trigger('change');
				$('div.view_modal').modal('hide');
				toastr.success(result.msg);
			} else {
				toastr.error(result.msg);
