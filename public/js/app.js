$(document).ready(function(){
    ...
    //Rak table
    var rak_table = $('#rak_table').DataTable({
					processing: true,
					serverSide: true,
					ajax: '/simpan-barang',
					columnDefs: [ {
    ...
    $(document).on('submit', 'form#add_rak_form', function(e){
		e.preventDefault();
		var data = $(this).serialize();
		...
		$.ajax({
			...
			success: function(result){
				if(result.success == true){
					$('div.modal_rak_barang').modal('hide');
					toastr.success(result.msg);
					rak_table.ajax.reload();
				} else {
					toastr.error(result.msg);
				}
			}
	...
	$(document).on('click', 'button.edit_rak_button', function(){

    	$( "div.modal_rak_barang" ).load( $(this).data('href'), function(){

    		$(this).modal('show');

    		$('form#rak_edit_form').submit(function(e){
	    		e.preventDefault();
				var data = $(this).serialize();
    ...
    $(document).on('click', 'button.delete_rak_button', function(){
    	swal({
          title: LANG.sure,
