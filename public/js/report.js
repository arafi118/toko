$(document).ready( function(){
    ...
    stock_report_table = $('#stock_report_table').DataTable({
                            ...
                            columns: [
                                {data: 'DT_RowId', name: 'products.id'},
                                {data: 'sku', name: 'sku'},
                                {data: 'product', name: 'products.name'},
                                {data: 'unit_price', name: 'V.sell_price_inc_tax'},
                                {data: 'hpp_price', name: 'V.dpp_inc_tax'},
                                {data: 'price_group', name: 'products.id'},
    ...
    $('#stock_report_filter_form #location_id, #stock_report_filter_form #category_id, #stock_report_filter_form #sub_category_id, #stock_report_filter_form #brand, #stock_report_filter_form #rak_id, #stock_report_filter_form #unit,#stock_report_filter_form #view_stock_filter').change( function(){
    ...
    //Report sell periode
    if($('#report_sell_period_filter').length == 1){
        $('#report_sell_period_filter').daterangepicker(
            dateRangeSettings,
            function (start, end) {
                $('#report_sell_period_filter').val(start.format(moment_date_format) + ' ~ ' + end.format(moment_date_format));
                report_sell_period_table.ajax.reload();
                // $('#start_date').val(start.format(moment_date_format));
                // $('#end_date').val(end.format(moment_date_format));
            }
        );
        $('#report_sell_period_filter').on('cancel.daterangepicker', function(ev, picker) {
            $('#report_sell_period_filter').val('');
            report_sell_period_table.ajax.reload();
        });
        // $('#start_date').data('daterangepicker').setStartDate(moment());
        // $('#end_date').data('daterangepicker').setEndDate(moment());
        $('#report_sell_period_filter').data('daterangepicker').setStartDate(moment());
        $('#report_sell_period_filter').data('daterangepicker').setEndDate(moment());
        
    }
    $('#report_periode_sell_form #report_sell_period_filter').change( function(){
        report_sell_period_table.ajax.reload();
        let str_dt = $('input#report_sell_period_filter').data('daterangepicker').startDate.format('YYYY-MM-DD');
        let end_dt = $('input#report_sell_period_filter').data('daterangepicker').endDate.format('YYYY-MM-DD');
        $('#start_date').val(str_dt);
        $('#end_date').val(end_dt);
    });

    report_sell_period_table = $('table#sell_report_period').DataTable({
        processing: true,
        serverSide: true,
        aaSorting: [[3, 'desc']],
        "ajax": {
            "url": "/reports/laporan-sell-periode",
            "data": function ( d ) {
                var start = '';
                var end = '';
                // var start2 = '';
                // var end2 = '';
                if($('#report_sell_period_filter').val()){
                    // start2 = $('input#start_date').data('daterangepicker').startDate.format('YYYY-MM-DD');
                    // end2 = $('input#end_date').data('daterangepicker').endDate.format('YYYY-MM-DD');
                    start = $('input#report_sell_period_filter').data('daterangepicker').startDate.format('YYYY-MM-DD');
                    end = $('input#report_sell_period_filter').data('daterangepicker').endDate.format('YYYY-MM-DD');
                }
                d.start_date = start;
                d.end_date = end;
        ...
        columns: [
            { data: 'product_id', name: 'p.id'  },
            { data: 'product_name', name: 'p.name'  },
            // { data: 'supplier', name: 'c.name'  },
            { data: 'sku', name: 'p.sku'  },
            { data: 'sell_price', name: 'v.sell_price_inc_tax'},
            { data: 'tot_sold', name: 'tot_sold', searchable: false},
            // { data: 'action', name: 'action'},
