...
// 	});
	...
	$('#bayar_purchase').on('keyup', function () {
        //sads
        let sum_tot_barang = $('#sum_tot_barang').val();
        let bayar_purchase = $('#bayar_purchase').val();
        let bayar_purchase_conv = __number_uf(bayar_purchase);

        let tot_all = sum_tot_barang - bayar_purchase_conv;
		__write_number($('.payment-amount').first(), tot_all);

        if(bayar_purchase_conv > sum_tot_barang){
			toastr["warning"]("Jumlah Bayar Tidak Boleh Melebihi Jumlah Pembelian", "Peringatan")
		}
    });
    ...
    $('#is_hutang_piutang').change(function(){
        if($(this).is(':checked')){
            // ddasdas
            if(!$('.purchase_bayar_line').hasClass('hide')){
                $('.purchase_bayar_line').addClass('hide');
            }
            
            
            let sum_tot_barang = $('#sum_tot_barang').val();
            __write_number($('#amount_0'), sum_tot_barang, true);
        }else{
            // dsada
            $('.purchase_bayar_line').removeClass('hide');
            $('#bayar_purchase').val('');
        }
    });
    ...
    $('#supplier_id').select2({
        ...
        language: {
            noResults: function () {
                ...
                return '<button type="button" data-name="' + name + '" class="btn btn-link add_new_supplier"><i class="fa fa-plus-circle fa-lg" aria-hidden="true"></i>&nbsp; ' + __translate('add_name_as_new_supplier', {
                    'name': name
                }) + '</button>';
            }
        },
            ...
            $.ajax({
                ...
                success: function (result) {
                    if (result.success == true) {
                        $("select#supplier_id").append($('<option>', {
    ...
    $(document).on('focusout','.row_subtotal_before_tax_hidden', function() {
        var row = $(this).closest('tr');
        
        var quantity = __read_number(row.find('input.purchase_quantity'), true);
        var subtotal = __read_number($(this), true) 
        var purchase_unit = parseInt(subtotal) / parseInt(quantity)
        __write_number(row.find('.purchase_unit_cost_without_discount'), __currency_trans_from_en(purchase_unit), true);
    ...
    $('.quick_add_product_modal').on('shown.bs.modal', function () {
        $('.quick_add_product_modal').find('.select2').each(function () {
            $(this).select2({
                dropdownParent: $(".quick_add_product_modal")
            });
        });
...
function update_grand_total() {
    ...
    __write_number($('#amount_0'), grand_total, true);
    var uuuuu = $('#amount_0').val();
    var h = uuuuu.split(',');
    var res = h.join("");
    $('#sum_tot_barang').val(res);
    // __write_number($('#sum_tot_barang'), grand_total, false);
