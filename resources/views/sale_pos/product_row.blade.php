<tr class="product_row" data-row_index="{{ $row_count }}">
    <td>
        ...
        @if (session()->get('business.enable_lot_number') == 1 || session()->get('business.enable_product_expiry') == 1)
            ...
            @if (!empty($product->lot_numbers))
                <select class="form-control lot_number" name="products[{{ $row_count }}][lot_no_line_id]"
    ...
    <td>
        ...
        <input type="hidden" value="{{ $unit_price_inc_tax - $product->dpp_inc_tax }}"
            name="products[{{ $row_count }}][laba_product]" class="row_laba_product">

        <input type="hidden" value="{{ $product->dpp_inc_tax }}" name="products[{{ $row_count }}][dpp_inc_tax]"
            class="row_dpp_inc_tax">
        ...
        <div class="input-group input-number">
            ...
            <input data-variation-id="{{ $product->variation_id }}" data-product="{{ $product->product_id }}"
                type="text" data-min="1" class="form-control pos_quantity input_number mousetrap"
			...
			data-msg-abs_digit="@lang('lang_v1.decimal_value_not_allowed')" @endif
                ...
                @if ($product->enable_stock) data-rule-max-value="{{ $max_qty_rule }}"
			...
			$product->formatted_qty_available, 'unit' => $product->unit ])" @endif>
    ...
    <td class="text-center">
        <i data-variation="{{ $product->variation_id }}"
            class="fa fa-close text-danger pos_remove_row cursor-pointer" aria-hidden="true"></i>
    </td>
</tr>
