...
<div class="modal-dialog modal-xl no-print" role="document">
  <div class="modal-content">
    ...
    <h4 class="modal-title" id="modalTitle"> Detil Pre-Order (<b>@lang('sale.invoice_no'):</b> {{ $sell->invoice_no }})
    </h4>
</div>
    ...
    <div class="row">
      ...
      <div class="col-sm-4">
        ...
        if($sell->pay_term_type == 'days'){
          ...
          // dd($cek);
          $tgl = \Carbon::createFromFormat('Y-m-d H:i:s', $sell->transaction_date)->addDays($nom)->format('d M Y');
          // $tgl = \Carbon::today()->addDays($nom)->format('d M Y');
        } else{
          // $bulan = \Carbon::today()->addDays($nom)->toDateString();
          $tgl = \Carbon::createFromFormat('Y-m-d H:i:s', $sell->transaction_date)->addMonths($nom)->format('d M Y');
                ...
                //  $real_price = $trans->check_harga($sell->contact_id,$sell_line->variation_id);
                 ...
                 @endphp
                  <tr>
                    ...
                    <td>
                      <span class="display_currency" data-currency_symbol="true">{{ $real_price }}</span>
                    </td>
                    <td>
                      &nbsp;
                    </td>
                    ...
                    <td>
                      <span class="display_currency" data-currency_symbol="true">{{ $modifier->quantity * $real_price }}</span>
                    </td>
                  </tr>
                @endforeach
              @endif
