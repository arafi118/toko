...
<html>
	...
	<body>
							...
							<tbody>
								...
								<tr>
									<td class="style9" width="auto">
									    @php
									        $name = (isset($pengawas)) ? $pengawas['first_name'] .' '. $pengawas['last_name']:'';
									        $name = (!$name || $name == '' || $name == null || $name == ' ' ) ? '...................................':$name;
									    @endphp
										<center>( {{ $name }} )</center>
									</td>
								</tr>
		...
		<div class="style9" style="float: right; margin-right: 5px;">Dicetak oleh :{{$ttd}}, {{$bl->city}} ,
			{{ $date }} , {{date("h:i:s")}}</div>
	</body>
    <title>LAPORAN INVENTARIS BULAN {{$bln!=null ? strtoupper(array_search($bln, array_flip($bulan)))  : ''}} {{$thn!=null ? $thn : ''}}</title>
</html>