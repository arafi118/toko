...
<body>
	...
	<table width="96%" border="1" style="border-collapse: collapse;" align="center" cellpadding="3" cellspacing="0" class="style9">
        <tbody>
			...
			<tr>
				...
				?>
				<td>Saldo Awal Per {{$tgl!=null ? $tgl : '1'}} {{Request::get('bln')!=null ? strtoupper(array_search(Request::get('bln'), array_flip($bulan)))  : ''}} {{Request::get('thn')!=null ? Request::get('thn') : ''}}</td>
			...
			<tr>
				...
				<td>{{$v['nama_rekening']." : ".$v['invoice_no']."-".$v['nama_kontak']."-".$v['id_kontak']."-".$v['keterangan']}}</td>
				<td align="right">{{isset($v['kd_rekening_debit']) ? number_format($v['nominal'], 2) : 0}}</td>
				<td align="right">{{isset($v['kd_rekening_kredit']) ? number_format($v['nominal'], 2) : 0}}</td>
				<td align="right">
					<?php 
			...
			?>
			<tr>
				<td colspan="5">{{$tr_ini}} {{Request::get('bln')!=null ? ucwords(array_search(Request::get('bln'), array_flip($bulan)))  : ''}} {{Request::get('thn')!=null ? Request::get('thn') : ''}}</td>
			...
			<tr>
				<td colspan="5">{{$tr_sd_ini}} {{Request::get('bln')!=null ? ucwords(array_search(Request::get('bln'), array_flip($bulan)))  : ''}} {{Request::get('thn')!=null ? Request::get('thn') : ''}}</td>
				...
				<td align="center" rowspan="2" colspan="2"><b>{{number_format($saldo_awal_tahun + $saldo_awal_pakai + $saldo, 2)}}</b></td>
			</tr>
			<tr>
				<td colspan="5">Transaksi Komulatif sampai dengan Tahun {{Request::get('thn')!=null ? Request::get('thn') : ''}}</td>
				<td align="right"><b>{{number_format(($awal_pakai_debit + $t_debit) + $awal_tahun_debit, 2)}}</b></td>
				<td align="right"><b>{{number_format(($awal_pakai_kredit + $t_kredit) + $awal_tahun_kredit, 2)}}</b></td>
			</tr>
		</tbody>
	</table>
	...
	<tbody><tr>
		  ...
		  <td class="style27 align-center" valign="top" width="31%"><br>Dilaporkan Oleh:<br>{{$bl->name}}<br>
