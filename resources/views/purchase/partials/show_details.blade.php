            ...
            @if ($purchase->document)
                @php
                    $document_name = !empty(explode('_', $purchase->document, 2)[1])
                        ...
                        : $purchase->document;
                @endphp
                <a href="{{ url('storage/documents/' . $purchase->document) }}" download="{{ $document_name }}"
                ...
                <table class="table bg-gray">
                    ...
                    @foreach ($purchase->purchase_lines as $purchase_line)
                        <tr>
                            ...
                            <td>{{ number_format($purchase_line->quantity, 2) }}</td>
                            ...
                            <td class="no-print"><span class="display_currency"
                                    data-currency_symbol="true">{{ floatval($purchase_line->quantity) * $purchase_line->purchase_price }}</span>
                            </td>
                            <td><span class="display_currency"
                    ...
                    @endif
                    <td><span class="display_currency"
                            data-currency_symbol="true">{{ $purchase_line->purchase_price_inc_tax * floatval($purchase_line->quantity) }}</span>
                    </td>
                    </tr>
                    @php
                        $total_before_tax +=
                            floatval($purchase_line->quantity) * $purchase_line->purchase_price_inc_tax;
                    @endphp
                    @endforeach
                </table>
            </div>
    ...
    <div class="row">
        ...
        <div class="col-md-6 col-sm-12 col-xs-12">
            <div class="table-responsive">
                <table class="table">
                    <tr>
                        <th>@lang('purchase.amount_already_paid'): </th>
                        <td></td>
