...
class ProductUtil extends Util
{
    ...
    public function getDataProdukPopuler($business_id, $start, $end)
    {
        // dd($start);
        ...
        if (!empty($start) && !empty($end)) {
            ...
            $tgl_filter .= "AND DATE(transaction_date) BETWEEN '$start' AND '$end'";
        }
        // dd($tgl_filter);

        $populer = $query->select(
            'products.name as product',
            // DB::raw("(SUM(tsl.quantity)) as total_unit_sold"),
            DB::raw("(SELECT SUM(quantity) FROM transaction_sell_lines LEFT JOIN transactions ON transaction_sell_lines.transaction_id=transactions.id WHERE transactions.status='final' $tgl_filter AND transaction_sell_lines.product_id=products.id) as total_unit_sold"),
